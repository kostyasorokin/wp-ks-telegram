...
Each content type has its own switches for featured image, title, linked title, short description (from the excerpt, up to 160 characters), separate post link, and link preview. The original linked-title-only format remains the default. When enabled, a same-site featured image is sent with the chosen text as one photo caption. If the image is unavailable, or the caption exceeds Telegram's limit, a text message is sent instead. An image-only configuration without a usable image falls back to a linked title. The channel's silent-publishing switch is independent of administrator notifications.

Only the first transition to `publish` queues a channel message. Existing publications, later edits, revisions, autosaves, and password-protected posts are excluded. Turning off a content type or changing the channel before a queued message runs prevents delivery to the old destination. Delivery uses WordPress cron with up to three attempts and a per-post/per-channel sent marker. A working system cron calling `wp-cron.php` is recommended if delivery must be prompt even when the site has no visitors. If Telegram accepts a message but the response is lost, a retry can rarely produce a duplicate.
